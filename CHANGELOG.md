...
### R1.1 — one client, one context, one clock (2026-09-18)

The first slice of the release roadmap, and the end of the ~116 ms this project has carried as unexplained since M8. Design: [`docs/design/release-roadmap.md`](docs/design/release-roadmap.md) §2.1; record: [`docs/acceptance/one-client-run.md`](docs/acceptance/one-client-run.md) — `scripts/one-client-acceptance.sh`, **21 PASS, zero failures, third execution**. No contract change: all of this sits below the wire. Touches `agent`, `gateway`, `inference-driver`, `library` and `control`.

- **The cause was one line written 28 times.** Constructing an `httpx.AsyncClient()` without passing `verify=` builds a fresh `ssl.SSLContext` and parses certifi's PEM bundle — measured at **104–136 ms of synchronous CPU** in these repos' own virtualenvs, on the Python both installers provision — and because it is synchronous it runs on the event loop, so it also froze every concurrent stream in the process. Twenty of the twenty-eight sites built one per call; fourteen sat on paths that repeat. Each component now owns a `_http.py` holding one SSL context for the process, and the objects that make repeated calls own one client: the driver's engine, the gateway's routing table and driver clients, the agent's engine probe and its library reads, the control root's node and replication clients.
- **Measured against a stub backend that thinks for 5 ms:** control-plane overhead per request **3.2 ms**, where the driver hop alone was 110.3 ms and is now 2.0 ms; client construction 104–136 ms → **0.03 ms**; ten concurrent completions in **31.3 ms**, where ten client builds alone would have been about a second of blocked event loop. The agent's readiness loop was spending roughly 0.84 s of every 2 s on this with four resident runtimes — on the loop that also serves the UI and the browser proxy.
- **Every duration this project reports moved to `time.perf_counter()`** — 48 call sites. On Windows/CPython 3.12, which both installers provision, `time.monotonic()` is `GetTickCount64`: **20 distinct values in 300 ms**, a 15.6 ms grid under every `*_ms` the gateway reports. CPython fixed it in 3.13 and the agent's own virtualenv is 3.14, so the defect was invisible on the developer's machine and present on every shipped install. It is also why the original figure was obtained by subtracting two spans measured on that grid and came out as an unexplained 116 ms. The driver's non-streaming `latencyMs` now brackets the same span the streaming path does, rather than reporting `response.elapsed`, which starts after the client is built and so excluded the driver's largest cost from the driver's own measurement.
- **`trust_env` is decided per URL rather than per call site, and that is a correction to the fix as it was scoped.** With httpx's default, a user's `HTTP_PROXY` is applied to every request a component makes, including the ones this install makes to itself; on Windows the logon task inherits that variable from the user environment, so a corporate proxy that cannot reach 127.0.0.1 swallows every health probe. The poller gates the `starting → running` promotion rather than restarting anything, so the whole install sits at *starting* while actually serving, with no screen that explains it. But a blanket opt-out would have broken the two things here that legitimately reach the internet — the model hub and the GitHub release feed — and for a user behind a proxy that is not a slowdown but a total loss of discovery and download. The address decides: loopback, link-local, private ranges and a dotless hostname decline the proxy; everything else keeps it. The driver is why it cannot be a per-site rule — the same engine class fronts `127.0.0.1:8081` and `api.openai.com`.
- **The fix introduced a deadlock and the test suites hid it.** The first cut guarded the context cache with a non-reentrant `threading.Lock` and then called the function that takes the same lock from inside it: a hang on the first call of a process, and only the first, because every later caller finds the context already built and returns before the acquire. All 1,699 tests were green, because in any full run something had always built the context first. A cold agent would have hung on its first engine readiness probe, on every install. The lock is an `RLock`, and the check that catches it starts a fresh interpreter, since no test inside a warm process can be first.
- **43 new checks across the five repos**, plus `scripts/one-client-sabotage.py`: **twelve sabotages, all caught**, restoring from a copy and opening with a baseline assertion that the gates pass unsabotaged. One escaped at first and the sabotage was what was wrong — disabling either half of a double-checked lock changes nothing, because the other half still guards. The acceptance script's proxy check was sabotaged the same way and reproduced the silent install verbatim: both components reporting `starting` while up and serving, and the gateway answering that no models are routable.
- **Two harness defects, both the same shape as ones this project has recorded before.** The acceptance run first reported a *negative* overhead — the control plane faster than the backend it forwards to — because the backend floor was measured without connection pooling while the control plane uses it: two spans, two instruments, which is exactly how the 116 ms came to be unexplained. It now measures both ends with one pooled client and fails on a negative result rather than passing it. And its proxy check read a stale session token, minted before the restart it performs, as the proxy defect it exists to find.
- **Not in an install yet.** The five component repos change, nothing is re-pinned and the `dist` branch is untouched, because no UI and no contract changed. `engines/acquisition.py`'s synchronous `urlopen` with a 60-second timeout is still on the agent's event loop; that is a separate finding, scheduled as R1.5, and a shared SSL context does not help it.

### specs — two roadmap decisions taken, and a standing rule about re-verification (2026-09-18)

No contract change and no code. [`docs/design/release-roadmap.md`](docs/design/release-roadmap.md) is the order of work; two of its seven open decisions are now taken and five remain.

- **Anthropic `/v1/messages` goes in front of the release** (decision #1). It is scheduled as R4, alongside R2 and after R1 — not a feature competing with eleven High defects, but the one capability-matrix row that decides a first review, since llama-swap, llamactl and LM Studio all serve it and this project ships seven client recipes plus a test asserting Claude Code is *not* among them. **R4's first step is a measurement rather than code:** point a real Claude Code at a throwaway listener and capture the request, because the `x-api-key` premise the whole auth half rests on is upstream client behaviour that was reasoned about and never observed here.
- **The review's §4.3 re-ordering and re-wording of the callings is adopted** (decision #2), led by what `llama-server`'s router mode, llamactl and Unsloth Desktop do *not* do: installs, updates and restarts the engine for you → tells you what fits before you download, and starts your settings there → finds and downloads models in the app, into your own folders, as plain files → one endpoint for every tool you use → add the backends you already run and the subscriptions you already pay for → reach it from your other devices, safely → grows into a homelab.
- **The numbers did not move, and that is the substance of how it was applied: seven lines said, eight ideas numbered.** `differentiator #N` is cited by number in thirteen design documents, the acceptance records and the memory files, so renumbering would silently re-point a dozen citations that still read as true. **#4 keeps its number and becomes a mechanism** — the schema-driven config UI is how the fit line prefills a profile, not something said to a reader who has never heard of a schema — and the genuinely new calling, *one endpoint for every tool you use*, takes a new **#8** rather than #4's slot. **#8 is the one line on the list that is not true yet**; R4 is what makes it true, and it is marked as such everywhere it appears. The fifth and seventh lines are the two halves of #7. [`local-inference-control-plane.md`](docs/design/local-inference-control-plane.md) §2 carries both lists side by side and is the source of truth for the words; the README's hybrid — #1 and #7 re-worded on 2026-09-17 while four others kept the old wording, in the one document a stranger reads first — is closed.
- **`docs/private/` is now actually ignored.** The line that keeps the adversarial review out of this public repo was written the day the review was and had sat uncommitted, so the protection existed only in one working tree.
- **A standing rule, recorded in the roadmap at the top, in §1 and in the appendix: the findings do not get re-verified.** They were checked against the live clones hours after the last commit they describe. A session opening the roadmap executes it — it does not re-read the review to confirm a finding or re-grep to confirm a line number. This is a solo project; a second opinion costs a day and buys a sentence the document already contains. **It does not license skipping the reproduction check**, which every slice still ships and which is the fix's first half — that check is also what catches a coordinate that has drifted, in minutes rather than a pass. Three items the roadmap marks *unverified* are first verifications rather than repeats and are exempt.

### specs — pre-release adversarial review, and a roadmap (2026-09-17)

No contract change and no code. Record: `docs/private/adversarial-review-2026-09-17.md` (**gitignored — competitive notes and unfixed defects, deliberately not in the repo**); order of work: [`docs/design/release-roadmap.md`](docs/design/release-roadmap.md).

**This entry also marks a gap in this file.** The last entry before it is M9, dated 2026-09-11, and ten slices landed since without one: M10 token streaming, M11 compute/storage separation, install-paths steps 3-8 (installers, bootstrap, the container image, tool calling, context honesty, the playground diagnostic), tool calling and embeddings, Library folders and their reach, the resource-tree UI, the hobbyist onboarding plan S0-S10, Issues, and a node-local model copy. Each has a design document and an acceptance record; none has a changelog entry. Said here rather than implying continuity.

- **Three adversarial code passes over the six live repos** — security and trust, the request data path, and the first hour of install → model → run → UI — each required to anchor every finding to a file and line, to exclude the gaps the project already records, and to report what it *cleared* as well as what it found. **38 numbered findings plus one architectural note, 11 rated High for a public first release** (the review's own summary says 41; §6 enumerates 38 + §6.4). Every one was then re-verified against the working trees the same day: all still present, none already fixed, none misdiagnosed.
- **The headline is one root cause with a wide radius: every `httpx.AsyncClient()` in this project is built per call**, and constructing one parses certifi's PEM bundle — ~105 ms of synchronous CPU on the event loop, measured in four of the repos' own virtualenvs. **It is the ~116 ms of control-plane overhead M8 recorded and could not explain**, and it also burns ~0.21 s per ready runtime per poll — ~0.84 s of every 2 s with four resident — in the agent's runtime poller and ~0.6 s per gateway routing refresh. Two commitments were added to the project's immutable list as a result: one client per instance with `trust_env=False` on loopback, and `perf_counter` for anything reported as a duration — because on the Python both installers provision, Windows `monotonic()` has a 15.6 ms resolution, which is why the cost hid.
- **The second headline is that a fix landed on one of two code paths.** The per-layer KV arithmetic that corrected a 43× over-read at S6 lives in the library's preflight path; the on-disk fit route still builds its shape with the scalar reader — and that is the route one-click Run calls, so a starter model the recommendation card scores as *fits at 16,384* gets a profile written at ~4,864.
- **Also High:** a client disconnect mid-stream leaks the gateway's in-flight counters permanently (`CancelledError` is a `BaseException`, so neither cleanup branch runs), after which that runtime never idle-unloads; the login rate limiter can be driven from a request header, because the browser reaches login through our own proxy and the proxy forwards the forwarding headers; `GET /v1/engines` blocks the agent's event loop on `nvidia-smi` and a 60-second GitHub fetch every 15 seconds; `agent.yaml` is rewritten non-atomically and loaded unguarded; a taken port either dead-ends the wizard or produces a silently useless install with an empty Needs-attention card; one model launched on two nodes cross-wires status, lifecycle and attribution; a Windows machine with an AMD or Intel GPU is detected as having no GPU and handed a CPU-only engine build; and re-running the Windows installer elevated — which the installer itself suggests — creates a second install and strands the first.
- **What the review cleared, recorded because silence should be informative:** JWT verification and the audience model, route auth coverage, argv assembly, enrollment/rotation/epoch fencing, download integrity end to end, engine acquisition against live upstream, GGUF and safetensors parsing bounds, stream termination and tool-delta accumulation, the cascade taxonomy, wake de-duplication, metrics staying off the request path, and most of both installers.
- **The market half re-verified the field**, which had not been checked since 2026-09-11: the operations layer is no longer unclaimed (`llamactl` has shipped this thesis in Go since 2025-09 and released v0.21.2 that day), `llama-server`'s router mode took the single-host half of two differentiators *inside the engine*, and Unsloth Desktop is the beginner default on the strength of automatic hardware-aware settings. Three capabilities remain ours alone: replica balancing with tiered failover, cloud and subscription CLIs as peer backends, and multi-host as enrolled signed agents. Two rows decide the first review: **Anthropic `/v1/messages`**, which we do not serve, and *shipped a release*, which we have not.
- **Documentation corrected in place across nine files**, because five recorded conclusions were falsified: the 116 ms entry, a tier-numbering defect marked fixed that survives on the commonest slot shape, a routing window whose diagnosis named the narrow trigger, `degraded-mode-required` never having been applied to the agent's own state, and *"AMD, Intel and Apple detection unverified"* — which is not unverified but wrong in code, in the direction that runs out of memory. Two deployment documents described an agent the installers do not produce and were rewritten around what they do.
