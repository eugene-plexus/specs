...
| MLX (Apple Silicon engine) | **Unsupported in the shipped alpha.2**, which predates the integration. On current `main` (2026-09-22, toward alpha.3) MLX is integrated as an **experimental** engine with **simulated** evidence only: the [B1 identity run](acceptance/mlx-identity-run.md) proves routing, streaming, replicas, scopes and restart through real gateway/driver processes against MLX-shaped fixtures. Physical Apple silicon acceptance is **pending** ([the checklist](design/mlx-engine.md)). | The distinction that governs release: a known routing or shutdown defect is a blocker; missing Mac hardware evidence is a stated alpha testing limitation, not one. Do not describe experimental Mac support as physically verified before that evidence exists. |
| Typed decisions (`POST /v1/systemone`), supervised Kev | **Unsupported in the shipped alpha.2.** On current `main` (2026-09-22, toward alpha.3): **Measured** on WSL2 CPU — the [B2 run](acceptance/decision-run.md) had a real agent spawn the pinned `jaredpalmer/kev-0.8b`, a real gateway route real `curl` decisions to it, offline relaunch included. The engine is **experimental**: CUDA, ROCm and Metal are **pending** their own evidence. **Hosted Jev is unverified** — fixture-covered only until one real credentialed request is recorded; it is never a local fallback. | TypeSafe protocol compatibility only; Vercel `/v1/evaluate` / AI SDK evaluation compatibility is not claimed. PyPI's `typesafe` package is unrelated to TypeSafe's SDK (measured). |
